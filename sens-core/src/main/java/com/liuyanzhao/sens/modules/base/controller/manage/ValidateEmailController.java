...
import com.liuyanzhao.sens.common.vo.EmailValidate;
import com.liuyanzhao.sens.modules.base.vo.OtherSetting;
import com.liuyanzhao.sens.common.vo.Result;
import com.liuyanzhao.sens.modules.base.vo.VaptchaSetting;
...
public class ValidateEmailController {
    ...
    public VaptchaSetting getVaptchaSetting(){
        ...
        if(StrUtil.isBlank(v)){
            throw new SensException("系统还未配置Vaptcha验证码");
        }
    ...
    public OtherSetting getOtherSetting(){
        ...
        if(StrUtil.isBlank(v)){
            throw new SensException("系统未配置访问域名");
        }
    ...
    public Result<Object> sendCodeEmail(@PathVariable String email,
        ...
        if (StrUtil.isBlank(tokenLimit)) {
            throw new SensException("您发送的太频繁啦，请稍后再试");
        }
        // 生成验证码 存入相关信息
        ...
        emailUtil.sendTemplateEmail(email, "【SENS】修改邮箱验证","code-email", e);
    ...
    public Result<Object> sendResetCode(@RequestParam String email,
        ...
        emailUtil.sendTemplateEmail(email, "【SENS】重置密码邮箱验证","code-email", e);
        return new ResultUtil<Object>().setData(null);
    }
