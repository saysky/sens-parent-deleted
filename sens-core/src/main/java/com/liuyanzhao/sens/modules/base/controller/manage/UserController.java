...
import com.liuyanzhao.sens.common.aync.AddMessage;
...
public class UserController {
    ...
    public Result<Object> smsLogin(@RequestParam String mobile,
        ...
        if(StrUtil.isBlank(v)){
            throw new SensException("验证码失效或KEY不正确");
        }
        if(!code.equals(v)){
            throw new SensException("验证码不正确");
        }
        User u = userService.findByMobile(mobile);
        if(u==null){
            throw new SensException("手机号不存在");
        }
