package com.liuyanzhao.sens.config.interceptor;

import com.liuyanzhao.sens.common.annotation.RateLimiter;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Value("${sens.rateLimit.enable}")
    private boolean rateLimitEnable;

    @Value("${sens.rateLimit.limit}")
    private Integer limit;

    @Value("${sens.rateLimit.timeout}")
        ...
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (StrUtil.isBlank(token3)) {
                    throw new SensException("当前访问人数太多啦，请稍后再试");
                }
            }
        }catch (Exception e){
